...
DEVTYPE_DATA	DeviceTable[PPJOY_MAX_DEVTYPE+1]=
				{
					{"NULL"},
					{DEVTYPE_JOYSTICK_NAME},
					{DEVTYPE_GENESIS_NAME},
					{DEVTYPE_SNESPAD_NAME},
					{DEVTYPE_PSXPAD_NAME},
					{DEVTYPE_IOCTL_NAME},
					{DEVTYPE_RETIRED_NAME},
					{DEVTYPE_RADIOTX_NAME}
				};
...
/*					Joystick interface name		DevType				Units	Axes	Buttons */
					{"NULL",					0,					0,		0,		0},			/* 0 */
					{IF_THEMAZE_NAME,			IF_THEMAZE_DEV,		1,		2,		3},			/* 1 */
					{IF_IANHARRIES_NAME,		IF_IANHARRIES_DEV,	1,		2,		1},			/* 2 */
					{IF_TURBOGRAFX_NAME,		IF_TURBOGRAFX_DEV,	7,		2,		5},			/* 3 */
					{IF_LINUX0802_NAME,			IF_LINUX0802_DEV,	2,		2,		1},			/* 4 */
					{IF_LINUXDB9C_NAME,			IF_LINUXDB9C_DEV,	1,		2,		8},			/* 5 */
					{IF_TORMOD_NAME,			IF_TORMOD_DEV,		1,		2,		4},			/* 6 */
					{IF_DIRECTPRO_NAME,			IF_DIRECTPRO_DEV,	1,		2,		2},			/* 7 */
					{IF_TURBOGFX45_NAME,		IF_TURBOGFX45_DEV,	7,		2,		5},			/* 8 */
					{IF_LPTJOYSTICK_NAME,		IF_LPTJOYSTICK_DEV,	1,		2,		1},			/* 9 */
					{IF_CHAMPGAMES_NAME,		IF_CHAMPGAMES_DEV,	1,		2,		1},			/* 10 */
					{IF_STFORMAT_NAME,			IF_STFORMAT_DEV,	2,		2,		1},			/* 11 */
					{IF_SNESKEY2600_NAME,		IF_SNESKEY2600_DEV,	1,		2,		1},			/* 12 */
					{IF_AMIGA4PLAYER_NAME,		IF_AMIGA4PLAYER_DEV,2,		2,		2},			/* 13 */
					{IF_PCAE_NAME,				IF_PCAE_DEV,		2,		2,		1},			/* 14 */
					{IF_GENESISLIN_NAME,		IF_GENESISLIN_DEV,	1,		2,		8},			/* 15 */
					{IF_GENESISDPP_NAME,		IF_GENESISDPP_DEV,	1,		2,		8},			/* 16 */
					{IF_GENESISNTP_NAME,		IF_GENESISNTP_DEV,	2,		2,		8},			/* 17 */
					{IF_SNESPADLIN_NAME,		IF_SNESPADLIN_DEV,	5,		2,		12},		/* 18 */
					{IF_SNESPADDPP_NAME,		IF_SNESPADDPP_DEV,	5,		2,		12},		/* 19 */
					{IF_GENESISCC_NAME,			IF_GENESISCC_DEV,	2,		2,		8},			/* 20 */
					{IF_GENESISSNES_NAME,		IF_GENESISSNES_DEV,	1,		2,		8},			/* 21 */
					{IF_PSXPBLIB_NAME,			IF_PSXPBLIB_DEV,	8,		4,		16},		/* 22 */
					{IF_PSXDPADPRO_NAME,		IF_PSXDPADPRO_DEV,	2,		4,		16},		/* 23 */
					{IF_PSXLINUX_NAME,			IF_PSXLINUX_DEV,	5,		4,		16},		/* 24 */
					{IF_PSXNTPADXP_NAME,		IF_PSXNTPADXP_DEV,	6,		4,		16},		/* 25 */
					{IF_PSXMEGATAP_NAME,		IF_PSXMEGATAP_DEV,	5,		4,		16},		/* 26 */
					{IF_IOCTL_NAME,				IF_IOCTL_DEV,		16,		8,		16},		/* 27 */
					{IF_LINUXGAMECON_NAME,		IF_LINUXGAMECON_DEV,5,		2,		2},			/* 28 */
					{IF_LPTSWITCH_NAME,			IF_LPTSWITCH_DEV,	2,		2,		30},		/* 29 */
					{IF_FMSBUDDYBOX_NAME,		IF_FMSBUDDYBOX_DEV,	1,		0,		0},			/* 30 */
					{IF_POWERPADLIN_NAME,		IF_POWERPADLIN_DEV,	2,		0,		0},			/* 31 */
					{IF_GENESISDPP6_NAME,		IF_GENESISDPP6_DEV,	1,		2,		8}			/* 32 */
				};
...
char *GetDevTypeName(UCHAR DevType)
{
...
char *GetJoyTypeName (UCHAR JoyType)
{
 if (JoyType>PPJOY_MAX_IFTYPE)
  return ("UnknownIF");
...
// DeviceInfoSet= SetupDiGetClassDevs ((LPGUID)&GUID_PARALLEL_DEVICE,NULL,NULL,DIGCF_DEVICEINTERFACE);
 ...
 while (1)
 {
  ...
  if (!LPTNumber)
  {
   DebugPrintf (("Port ignored - not a valid LPT name\n"))
   continue;
  }
  if (LPTNumber>ArraySize)
  {
   DebugPrintf (("Port ignored - LPT number (%d) is larger then array size (%d)\n",LPTNumber,ArraySize))
   continue;
  }
...
int OpenPPJoyBusDriver (void)
{
 ...
 else
 {
  ...
  {
   DebugPrintf (("SetupDiGetDeviceInterfaceDetail() error %d\n",GetLastError()))
  }
  else
  {
...
int AddJoystick (UCHAR LPTNumber, UCHAR JoyType, UCHAR UnitNumber, UCHAR JoySubType, USHORT VendorID, USHORT ProductID)
{
 ...
 DebugPrintf (("Adding joystick type 0x%X on port 0x%X (%s persistent)\n",AddData.JoyData.JoyType,AddData.JoyData.LPTNumber,AddData.Persistent?"":"not "))
...
ULONG GetLptNumber (char *LptName)
{
 ULONG	Number;
...
int SetPPJoyOption (ULONG Option, ULONG Value)
{
 ...
 DebugPrintf (("Successfully set joystick options\n"))
 return 1;
}
